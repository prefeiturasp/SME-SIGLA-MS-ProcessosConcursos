...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'Erro ao consultar API: {exc}'))
            return

        if dry_run:
            self.stdout.write(self.style.WARNING('Execução em modo dry-run; nada será gravado.'))
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Cargos -> criados: {created_cargos}, atualizados: {updated_cargos}')
                )
                ...
                for item in concursos_api:
                    ...
                    # Vincular cargos (M2M)
                    cargos_objs = [codigo_to_cargo[c] for c in cargos_codes if c in codigo_to_cargo]
                    if cargos_objs:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Concursos -> criados: {created_conc}, atualizados: {updated_conc}, '
                        f'm2m vinculados: {linked_m2m}'
                    )
                )

                if dry_run:
                    self.stdout.write(self.style.WARNING('Dry-run: revertendo transação.'))
            ...
            self.stderr.write(self.style.ERROR(f'Erro ao persistir no banco: {exc}'))
            return
