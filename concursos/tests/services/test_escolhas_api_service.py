...
def test_get_escolhas_por_cargo_success():
    service = EscolhasAPIService(base_url='http://example.com', timeout_seconds=5)
    fake_response = Mock()
    fake_response.json.return_value = {"1001": 5, "1002": 4}
    fake_response.raise_for_status.return_value = None
    with patch('concursos.services.escolhas_api_service.requests.get', return_value=fake_response) as mocked_get:
...
def test_get_escolhas_por_cargo_http_error_propagates():
    ...
    with patch('concursos.services.escolhas_api_service.requests.get', return_value=fake_response):
